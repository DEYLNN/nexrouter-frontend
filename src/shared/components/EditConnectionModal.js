...
export default function EditConnectionModal({ isOpen, connection, onSave, onClose }) {
  ...
  const handleLiveModelProbe = async () => {
    if (!connection?.id || !liveProbeModel) return;
    setLiveProbeTesting(true);
    setLiveProbeResult(null);
    try {
      const res = await fetch(`/api/providers/${connection.id}/test-model`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: liveProbeModel }),
      });
      const data = await res.json();
      setLiveProbeResult(data);
    } catch (error) {
      setLiveProbeResult({ ok: false, error: error.message || "Test failed" });
    } finally {
      setLiveProbeTesting(false);
    }
  };
                ...
                <div className="mt-3 rounded-2xl border border-primary/15 bg-surface/70 p-3">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-text-muted">Runs a real chat completion using this exact {liveProbeProviderName} key.</p>
                    </div>
                    {liveProbeResult && (
                      <Badge variant={liveProbeResult.ok ? "success" : "error"}>
                        {liveProbeResult.ok ? "Working" : "Failed"}
                      </Badge>
                    )}
                  </div>
                  ...
                  {liveProbeResult && (
                    <div className="mt-2 rounded-xl border border-border bg-bg p-3 text-xs">
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-text-muted">
                        <span>Status: <b className={liveProbeResult.ok ? "text-green-600" : "text-red-600"}>{liveProbeResult.status || "ERR"}</b></span>
                        {liveProbeResult.latencyMs !== undefined && <span>Latency: {liveProbeResult.latencyMs}ms</span>}
                      </div>
                      {liveProbeResult.content && <p className="mt-2 text-text-main">Response: {liveProbeResult.content}</p>}
                      {liveProbeResult.error && <p className="mt-2 break-words text-red-600">{liveProbeResult.error}</p>}
                    </div>
                  )}
                </div>
              )}
